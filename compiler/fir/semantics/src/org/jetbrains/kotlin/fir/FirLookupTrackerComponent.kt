...
abstract class FirLookupTrackerComponent : FirSessionComponent {
    ...
    abstract fun recordTypeLookup(name: String, inScope: String, source: KtSourceElement?, fileSource: KtSourceElement?)
}
...
fun FirLookupTrackerComponent.recordClassLikeLookup(classId: ClassId, source: KtSourceElement?, fileSource: KtSourceElement?, isTypeLookup: Boolean = true) {
    if (!classId.isLocal && classId !in StandardClassIds.allBuiltinTypes) {
        val classFqName = classId.asSingleFqName()
        if (isTypeLookup) {
            recordTypeLookup(classFqName.shortName().asString(), classFqName.parent().asString(), source, fileSource)
        } else {
...
fun FirLookupTrackerComponent.recordTypeResolveAsLookup(typeRef: FirTypeRef, source: KtSourceElement?, fileSource: KtSourceElement?, isTypeLookup: Boolean = true) {
    if (typeRef !is FirResolvedTypeRef) return // TODO: check if this is the correct behavior
    recordTypeResolveAsLookup(typeRef.type, source, fileSource, isTypeLookup)
}
...
fun FirLookupTrackerComponent.recordUserTypeRefLookup(typeRef: FirUserTypeRef, inScopes: Iterable<String>, fileSource: KtSourceElement?) {
    inScopes.forEach { scope ->
        typeRef.qualifier.fold(FqName(scope)) { result, suffix ->
            recordTypeLookup(suffix.name.asString(), result.asString(), typeRef.source, fileSource)
...
fun FirLookupTrackerComponent.recordTypeResolveAsLookup(type: ConeKotlinType?, source: KtSourceElement?, fileSource: KtSourceElement?, isTypeLookup: Boolean = true) {
    if (type == null) return
    ...
    type.classId?.let { classId ->
        recordClassLikeLookup(classId, source, fileSource, isTypeLookup)
    }
    type.typeArguments.forEach {
        if (it is ConeKotlinType) recordTypeResolveAsLookup(it, source, fileSource, isTypeLookup = true)
    }
}
